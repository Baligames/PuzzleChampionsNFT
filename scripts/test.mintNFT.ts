...
async function main() {
    // 프록시 컨트랙트 주소 설정 (PuzzleChampionsNFT 프록시 주소)
    ...
    //const tx_grant = await PuzzleChampionsNFT__factory.connect(proxyAddress, deployer).grantRole(MINTER_ROLE, minter.address);
    //const tx_receipt = await tx_grant.wait()
    //console.log("MINTER_ROLE granted to", tx_receipt);
  
    // PuzzleChampionsNFT 컨트랙트 인스턴스 생성
